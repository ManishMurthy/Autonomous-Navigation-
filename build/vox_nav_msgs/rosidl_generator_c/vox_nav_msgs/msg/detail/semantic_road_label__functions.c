// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from vox_nav_msgs:msg/SemanticRoadLabel.idl
// generated code does not contain a copyright notice
#include "vox_nav_msgs/msg/detail/semantic_road_label__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"


bool
vox_nav_msgs__msg__SemanticRoadLabel__init(vox_nav_msgs__msg__SemanticRoadLabel * msg)
{
  if (!msg) {
    return false;
  }
  // structure_needs_at_least_one_member
  return true;
}

void
vox_nav_msgs__msg__SemanticRoadLabel__fini(vox_nav_msgs__msg__SemanticRoadLabel * msg)
{
  if (!msg) {
    return;
  }
  // structure_needs_at_least_one_member
}

bool
vox_nav_msgs__msg__SemanticRoadLabel__are_equal(const vox_nav_msgs__msg__SemanticRoadLabel * lhs, const vox_nav_msgs__msg__SemanticRoadLabel * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // structure_needs_at_least_one_member
  if (lhs->structure_needs_at_least_one_member != rhs->structure_needs_at_least_one_member) {
    return false;
  }
  return true;
}

bool
vox_nav_msgs__msg__SemanticRoadLabel__copy(
  const vox_nav_msgs__msg__SemanticRoadLabel * input,
  vox_nav_msgs__msg__SemanticRoadLabel * output)
{
  if (!input || !output) {
    return false;
  }
  // structure_needs_at_least_one_member
  output->structure_needs_at_least_one_member = input->structure_needs_at_least_one_member;
  return true;
}

vox_nav_msgs__msg__SemanticRoadLabel *
vox_nav_msgs__msg__SemanticRoadLabel__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  vox_nav_msgs__msg__SemanticRoadLabel * msg = (vox_nav_msgs__msg__SemanticRoadLabel *)allocator.allocate(sizeof(vox_nav_msgs__msg__SemanticRoadLabel), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(vox_nav_msgs__msg__SemanticRoadLabel));
  bool success = vox_nav_msgs__msg__SemanticRoadLabel__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
vox_nav_msgs__msg__SemanticRoadLabel__destroy(vox_nav_msgs__msg__SemanticRoadLabel * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    vox_nav_msgs__msg__SemanticRoadLabel__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
vox_nav_msgs__msg__SemanticRoadLabel__Sequence__init(vox_nav_msgs__msg__SemanticRoadLabel__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  vox_nav_msgs__msg__SemanticRoadLabel * data = NULL;

  if (size) {
    data = (vox_nav_msgs__msg__SemanticRoadLabel *)allocator.zero_allocate(size, sizeof(vox_nav_msgs__msg__SemanticRoadLabel), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = vox_nav_msgs__msg__SemanticRoadLabel__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        vox_nav_msgs__msg__SemanticRoadLabel__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
vox_nav_msgs__msg__SemanticRoadLabel__Sequence__fini(vox_nav_msgs__msg__SemanticRoadLabel__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      vox_nav_msgs__msg__SemanticRoadLabel__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

vox_nav_msgs__msg__SemanticRoadLabel__Sequence *
vox_nav_msgs__msg__SemanticRoadLabel__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  vox_nav_msgs__msg__SemanticRoadLabel__Sequence * array = (vox_nav_msgs__msg__SemanticRoadLabel__Sequence *)allocator.allocate(sizeof(vox_nav_msgs__msg__SemanticRoadLabel__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = vox_nav_msgs__msg__SemanticRoadLabel__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
vox_nav_msgs__msg__SemanticRoadLabel__Sequence__destroy(vox_nav_msgs__msg__SemanticRoadLabel__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    vox_nav_msgs__msg__SemanticRoadLabel__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
vox_nav_msgs__msg__SemanticRoadLabel__Sequence__are_equal(const vox_nav_msgs__msg__SemanticRoadLabel__Sequence * lhs, const vox_nav_msgs__msg__SemanticRoadLabel__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!vox_nav_msgs__msg__SemanticRoadLabel__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
vox_nav_msgs__msg__SemanticRoadLabel__Sequence__copy(
  const vox_nav_msgs__msg__SemanticRoadLabel__Sequence * input,
  vox_nav_msgs__msg__SemanticRoadLabel__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(vox_nav_msgs__msg__SemanticRoadLabel);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    vox_nav_msgs__msg__SemanticRoadLabel * data =
      (vox_nav_msgs__msg__SemanticRoadLabel *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!vox_nav_msgs__msg__SemanticRoadLabel__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          vox_nav_msgs__msg__SemanticRoadLabel__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!vox_nav_msgs__msg__SemanticRoadLabel__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
