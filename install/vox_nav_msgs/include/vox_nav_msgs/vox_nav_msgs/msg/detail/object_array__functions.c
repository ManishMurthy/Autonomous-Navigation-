  ...
  vox_nav_msgs__msg__ObjectArray__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    vox_nav_msgs__msg__ObjectArray * data =
      (vox_nav_msgs__msg__ObjectArray *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!vox_nav_msgs__msg__ObjectArray__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          vox_nav_msgs__msg__ObjectArray__fini(&output->data[i]);
        }
        return false;
      }
    }
