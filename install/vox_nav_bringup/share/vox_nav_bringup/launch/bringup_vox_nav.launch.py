...
def generate_launch_description():
    ...
    use_transforms = LaunchConfiguration('use_transforms')
    ...
    declare_rviz_config = DeclareLaunchArgument(
        'rviz_config',
        default_value=os.path.join(
            share_dir, 'rviz', 'vox_nav_default_view.rviz'),
        description='Path to the rviz config file.')
    
    declare_use_transforms = DeclareLaunchArgument(
        'use_transforms',
        default_value='True',
        description='Whether to publish static transforms for VoxNav')

    utm_to_map_transform = Node(
        package='tf2_ros',
        executable='static_transform_publisher',
        name='utm_to_map_publisher',
        arguments=['0', '0', '0', '0', '0', '0', 'utm', 'map'],
        output='screen',
        condition=IfCondition(use_transforms)
    )
    
    # Additional safety transform for map to odom if needed
    ...
    traversable_cloud_relay = Node(
        package='topic_tools',
        executable='relay',
        name='traversable_cloud_relay',
        arguments=['/traversable_pointcloud', '/traversable_cloud'],
        output='screen'
    )

    # Topic relay for VoxNav cmd_vel to robot cmd_vel
    cmd_vel_relay = Node(
        package='topic_tools',
        executable='relay',
        name='voxnav_cmd_vel_relay',
        arguments=['/vox_nav/cmd_vel', '/cmd_vel'],
        output='screen'
    )
    ...
    return LaunchDescription([
        ...
        declare_use_transforms,
        

        utm_to_map_transform,
        map_to_odom_transform,

        # VoxNav core system
        bringup_cmd_group,
        
        # Movement bridge (launched last)
        cmd_vel_relay,
        traversable_cloud_relay,
    ])
